...
public class YoloSearch extends AppCompatActivity implements CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    public void YOLO(View Button) {
        ...
        if (startYolo == false) {
            ...
            if (firstTimeYolo == false) {
                ...
                /*String tinyYoloCfg = "vivo 1806/Transcend SD card/dnns/yolov3-tiny.cfg";
                String tinyYoloWeights = "vivo 1806/Transcend SD card/dnns/yolov3-tiny.weights";*/
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_yolo_search);
        ...
        string = startingIntent.getStringExtra("object").toLowerCase().replace(" ","");
            ...
            if (ArrayLength >= 1) {
                ...
                for (int i = 0; i < ind.length; ++i) {
                    ...
                    /*if (intConf > 30) {
                        ...
                        if (set.contains(cocoNames.get(idGuy))) {
                            ;
                        } else {
                            set.add(cocoNames.get(idGuy));
                            tts.speak(cocoNames.get(idGuy), TextToSpeech.QUEUE_ADD, null, "DEFAULT");
                        }
                    }*/

                    if(cocoNames.get(idGuy).toLowerCase().replace(" ","").indexOf(string)!= -1){
                        Imgproc.putText(frame, cocoNames.get(idGuy) + " " + intConf + "%", box.tl(), Core.FONT_HERSHEY_SIMPLEX, 2, new Scalar(255, 255, 0), 2);
                        Imgproc.rectangle(frame, box.tl(), box.br(), new Scalar(255, 0, 0), 2);
                        tts.speak(cocoNames.get(idGuy) + "Found", TextToSpeech.QUEUE_ADD, null, "DEFAULT");
                        /*last = list.get(list.size() - 1);
                        if (!last.equals(cocoNames.get(idGuy))) {
                            list.add(cocoNames.get(idGuy));
                            tts.speak(cocoNames.get(idGuy) + "Found", TextToSpeech.QUEUE_ADD, null, "DEFAULT");
                        }*/
                    }
                    else{
